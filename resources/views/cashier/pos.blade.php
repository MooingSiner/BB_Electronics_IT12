...
<div class="flex h-full">
    ...
    <div class="flex-1 flex flex-col bg-white border-r border-slate-200">
        ...
        <div class="p-4 border-b border-slate-200 space-y-3">
            ...
            <div class="flex gap-2 flex-wrap">
                @php
                    $categories = ['All', 'Lighting', 'Components', 'Switches', 'Wiring', 'Adapters', 'Batteries'];
                    $activeCategory = request('category', 'All');
                @endphp
                @foreach($categories as $cat)
                <a href="{{ route('cashier.pos', array_merge(request()->except('category'), ['category' => $cat === 'All' ? null : $cat, 'search' => request('search')])) }}"
                   class="px-3 py-1 rounded-full text-xs font-medium transition-colors
                   {{ ($activeCategory === $cat || ($cat === 'All' && !request('category')))
                      ? 'text-white'
                      : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}"
                   @if($activeCategory === $cat || ($cat === 'All' && !request('category')))
        ...
        <div class="flex-1 overflow-y-auto p-4">
            <div class="grid grid-cols-3 gap-3">
                ...
                <div class="bg-white border border-slate-200 rounded-xl p-3 relative transition-shadow
                    ...
                    <form method="POST" action="{{ route('cashier.pos.add') }}">
                        ...
                        <button type="submit" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" tabindex="-1" aria-label="Add {{ $product->name }} to cart"></button>
                    </form>
                    @endif
    ...
    <div class="w-96 flex flex-col bg-slate-50 border-l border-slate-200">
        ...
        <div class="p-4 border-b border-slate-200 flex items-center justify-between">
            ...
            <form method="POST" action="{{ route('cashier.pos.clear') }}">
                @csrf
                <button type="submit" class="text-red-500 text-sm hover:underline transition-colors">Clear Cart</button>
            </form>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-2">
            ...
            <div class="bg-white rounded-lg p-3 border border-slate-200">
                <div class="flex items-start justify-between mb-2">
                    ...
                    <form method="POST" action="{{ route('cashier.pos.remove') }}">
                        @csrf
                        <input type="hidden" name="cart_key" value="{{ $cartKey }}">
                        <button type="submit" class="text-slate-400 hover:text-red-500 transition-colors flex-shrink-0">
        ...
        <div class="p-4 border-t border-b border-slate-200">
            ...
            <form method="POST" action="{{ route('cashier.pos.discount') }}">
                @csrf
                <div class="flex gap-2 mb-3">
                    ...
                    <button type="submit"
                            ...
                            {{ session('discount_type', 'none') === $val
                               ? 'text-white border-transparent'
                               : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50' }}"
                            @if(session('discount_type', 'none') === $val)
                            style="background-color:#363E48;"
                            @endif>
                        {{ $label }}
                    </button>
                ...
                @if(session('discount_type', 'none') !== 'none')
                <div class="flex gap-2">
                    <input type="number"
                           name="discount_value"
                           value="{{ session('discount_value', '') }}"
                           min="0"
                           step="0.01"
                           placeholder="{{ session('discount_type') === 'percent' ? 'e.g. 10' : 'e.g. 50.00' }}"
                           class="flex-1 px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2"
                           style="--tw-ring-color:#363E48;">
                    <button type="submit"
                            name="apply_discount"
                            value="1"
                            class="px-3 py-2 text-xs font-medium text-white rounded-lg transition-opacity hover:opacity-90"
                            style="background-color:#363E48;">
                        Apply
                    </button>
                </div>
        ...
        <div class="p-4 bg-white border-t border-slate-200 space-y-1.5 text-sm">
            ...
            <p class="text-xs text-slate-400">VAT (12% included): ₱{{ number_format($vat ?? 0, 2) }}</p>
        </div>
        ...
        <div class="p-4 border-t border-slate-200 space-y-3">
            ...
            <form method="POST" action="{{ route('cashier.pos.payment') }}">
                @csrf
                <div class="flex gap-2 mb-3">
                    ...
                    <button type="submit"
                            ...
                            value="{{ $method }}"
                            class="flex-1 py-1.5 rounded-lg text-xs font-medium border transition-colors
                            {{ session('payment', 'Cash') === $method
                               ? 'text-white border-transparent'
                               : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50' }}"
                            @if(session('payment', 'Cash') === $method)
                            style="background-color:#363E48;"
                            @endif>
                        {{ $method }}
                    </button>
            ...
            @if(session('payment', 'Cash') === 'Cash')
            <form method="POST" action="{{ route('cashier.pos.amount') }}">
                @csrf
            ...
            {{-- Complete Sale --}}
            <form method="POST" action="{{ route('cashier.pos.complete') }}">
                @csrf
                <input type="hidden" name="payment_method" value="{{ session('payment', 'Cash') }}">
                <input type="hidden" name="discount_type" value="{{ session('discount_type', 'none') }}">
                <input type="hidden" name="discount_value" value="{{ session('discount_value', 0) }}">
                <input type="hidden" name="amount_received" value="{{ session('amount_received', 0) }}">
                @foreach(session('cart', []) as $cartKey => $item)
                <input type="hidden" name="cart[{{ $cartKey }}][product_id]" value="{{ $item['product_id'] }}">
                <input type="hidden" name="cart[{{ $cartKey }}][quantity]" value="{{ $item['quantity'] }}">
                <input type="hidden" name="cart[{{ $cartKey }}][price]" value="{{ $item['price'] }}">
                @endforeach
                <button type="submit"
                        class="w-full py-3 rounded-xl font-bold text-sm text-white transition-opacity mt-1
                        {{ count(session('cart', [])) === 0 ? 'opacity-40 cursor-not-allowed' : 'hover:opacity-90' }}"
                        style="background-color:#363E48;"
                        {{ count(session('cart', [])) === 0 ? 'disabled' : '' }}>
                    Complete Sale
                </button>
            </form>
        </div>
...
<div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
    <div class="bg-white rounded-2xl p-8 max-w-sm w-full mx-4 shadow-xl text-center">
        ...
        <p class="text-sm text-slate-500 mb-4">Transaction ID: {{ session('sale_txn_id') }}</p>
        <div class="text-left bg-slate-50 rounded-lg p-4 text-sm space-y-1 mb-4">
            <div class="flex justify-between">
                <span class="text-slate-500">Subtotal</span>
                <span>₱{{ session('sale_subtotal') }}</span>
            </div>
            @if(session('sale_discount') > 0)
            <div class="flex justify-between text-green-600">
                <span>Discount</span>
                <span>−₱{{ session('sale_discount') }}</span>
            </div>
            @endif
            <div class="flex justify-between font-bold">
                <span>Total</span>
                <span>₱{{ session('sale_total') }}</span>
            </div>
            <div class="flex justify-between">
                <span class="text-slate-500">Payment</span>
                <span>{{ session('sale_payment') }}</span>
            </div>
            @if(session('sale_payment') === 'Cash')
            <div class="flex justify-between">
                <span class="text-slate-500">Received</span>
                <span>₱{{ session('sale_received') }}</span>
            </div>
            <div class="flex justify-between">
                <span class="text-slate-500">Change</span>
                <span>₱{{ session('sale_change') }}</span>
            </div>
            @endif
            <div class="flex justify-between">
        ...
        <div class="space-y-2">
            <a href="{{ route('cashier.sales.receipt', session('sale_txn_id')) }}"
            ...
            <a href="{{ route('cashier.sales.show', session('sale_txn_id')) }}"
